...
export default function SignupPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    const res = await fetch("/api/v1/session/signup", {
      method: "POST",
    ...
    if (res.ok) {
      ...
      await fetch("/api/v1/session/auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, password }),
      });
  ...
  if (pending) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6">
        <div className="card-glass flex flex-col items-center gap-5 text-center px-8 py-10 fade-up" style={{ maxWidth: 400 }}>
          ...
          <h1 style={{ fontSize: 18, fontWeight: 600, color: "var(--text-primary)", letterSpacing: -0.3 }}>Request submitted</h1>
          <p style={{ fontSize: 13, color: "var(--text-tertiary)", lineHeight: 1.6 }}>
            Account <span style={{ color: "var(--accent-light)", fontWeight: 500 }}>{username}</span> is pending administrator approval.
          </p>
          <a href="/login" className="transition-colors hover:text-[var(--accent-light)]" style={{ marginTop: 8, fontSize: 13, color: "var(--text-muted)", textDecoration: "none" }}>
            Back to sign in
          </a>
        </div>
      </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6">
      ...
      <div className="card-glass fade-up" style={{
        ...
        <div className="flex justify-center gap-6">
          <Link href="/login" className="transition-colors hover:text-[var(--accent-light)]" style={{ fontSize: 13, color: "var(--text-tertiary)", textDecoration: "none" }}>
